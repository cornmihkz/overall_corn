---
title: "alangalang_insect_data_plant_hopper"
...
# convert date format
hopper_df_alangalang$collect_date <- 
  as.Date(hopper_df_alangalang$`Collection Date`, "%B %d, %Y")

hopper_dmg_alangalang <-
  hopper_df_alangalang %>% 
...
hopper_joined_alangalang$Variety %>% unique()
```

```{r}
hopper_alangalang_parsed <-
    hopper_joined_alangalang %>% 
    ...
    mutate(variety = case_when(str_detect(Variety, '\\b(OPV)') ~ "OPV White",
...
```{r}
cor.test(as.numeric(hopper_alangalang_rank_df$Mode_class_infest_hopper), as.numeric(hopper_alangalang_rank_df$rank_mean_temp), method="kendall")
```
